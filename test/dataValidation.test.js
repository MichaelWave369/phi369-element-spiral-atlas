...
test("electron configuration batch exists for elements 1 through 86",()=>{ for(let z=1;z<=86;z++){ const p=PROPERTY_SEEDS[z]; assert.equal(typeof p.electronConfiguration,"string"); assert.ok(p.electronConfiguration.length>0); assert.ok(Array.isArray(p.sourceRefs?.electronConfiguration)); p.sourceRefs.electronConfiguration.forEach((id)=>assert.ok(PROPERTY_SOURCES[id])); }});
test("electron configuration is not required beyond fourth batch",()=>{ for(let z=87;z<=120;z++){ const p=PROPERTY_SEEDS[z]; assert.ok(p); } const errors=runDataValidation({ elements:ELEMENTS, familyColors:FAMILY_COLORS, blockColors:BLOCK_COLORS, propertyPresets:PROPERTY_PRESETS, viewPresets:VIEW_PRESETS, harmonicFilters:HARMONIC_FILTERS, resonanceModes:RESONANCE_MODES, propertyMeta:PROPERTY_META, propertySeeds:PROPERTY_SEEDS, emptyElementProperties:EMPTY_ELEMENT_PROPERTIES, completenessFields:COMPLETENESS_FIELDS, propertySources:PROPERTY_SOURCES, digitalRoot }); assert.deepEqual(errors,[]);});

test("electron configuration exceptions/regression values are preserved",()=>{ assert.equal(PROPERTY_SEEDS[57].electronConfiguration,"[Xe] 5d1 6s2"); assert.equal(PROPERTY_SEEDS[58].electronConfiguration,"[Xe] 4f1 5d1 6s2"); assert.equal(PROPERTY_SEEDS[64].electronConfiguration,"[Xe] 4f7 5d1 6s2"); assert.equal(PROPERTY_SEEDS[71].electronConfiguration,"[Xe] 4f14 5d1 6s2"); assert.equal(PROPERTY_SEEDS[78].electronConfiguration,"[Xe] 4f14 5d9 6s1"); assert.equal(PROPERTY_SEEDS[79].electronConfiguration,"[Xe] 4f14 5d10 6s1"); assert.equal(PROPERTY_SEEDS[86].electronConfiguration,"[Xe] 4f14 5d10 6s2 6p6");});

...
test("electron configuration batch exists for elements 1 through 36",()=>{ for(let z=1;z<=36;z++){ const p=PROPERTY_SEEDS[z]; assert.equal(typeof p.electronConfiguration,"string"); assert.ok(p.electronConfiguration.length>0); assert.ok(Array.isArray(p.sourceRefs?.electronConfiguration)); p.sourceRefs.electronConfiguration.forEach((id)=>assert.ok(PROPERTY_SOURCES[id])); }});
test("electron configuration is not required beyond second batch",()=>{ for(let z=37;z<=120;z++){ const p=PROPERTY_SEEDS[z]; assert.ok(p); } const errors=runDataValidation({ elements:ELEMENTS, familyColors:FAMILY_COLORS, blockColors:BLOCK_COLORS, propertyPresets:PROPERTY_PRESETS, viewPresets:VIEW_PRESETS, harmonicFilters:HARMONIC_FILTERS, resonanceModes:RESONANCE_MODES, propertyMeta:PROPERTY_META, propertySeeds:PROPERTY_SEEDS, emptyElementProperties:EMPTY_ELEMENT_PROPERTIES, completenessFields:COMPLETENESS_FIELDS, propertySources:PROPERTY_SOURCES, digitalRoot }); assert.deepEqual(errors,[]);});

test("electron configuration exceptions/regression values are preserved",()=>{ assert.equal(PROPERTY_SEEDS[24].electronConfiguration,"[Ar] 3d5 4s1"); assert.equal(PROPERTY_SEEDS[29].electronConfiguration,"[Ar] 3d10 4s1"); assert.equal(PROPERTY_SEEDS[36].electronConfiguration,"[Ar] 3d10 4s2 4p6");});
